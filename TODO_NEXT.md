...
- [ ] Разобрать воспроизводимый сбой общего PTY-сценария
  `tuiide_integration_tests`: устарели маркер открытия File (`pen...`) и порядок
  Run/Project при обходе меню. Проверить ожидания диалогов и фокус редактора;
  сохранить проверки защиты изменённого файла и переключения проектов.
