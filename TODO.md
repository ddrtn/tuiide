...
## Новые доработки по результатам ручной проверки

- [x] Добавить управление CMake presets из диалогов Configure/Build: создание,
  клонирование, редактирование и удаление пользовательских presets. Показывать
  источник каждого элемента (`CMakePresets.json`, `CMakeUserPresets.json` или
  подключённый файл), сохранять пользовательские изменения атомарно и проверять
  ссылки `inherits` и build preset → configure preset перед изменением/удалением.
  Presets проекта и подключённых файлов по умолчанию оставлять только для чтения.
- [x] В диалоге добавления C++-класса предоставить два редактируемых поля:
  имя заголовочного файла и имя файла реализации. Изначально заполнять их теми
  именами, которые сейчас генерируются автоматически (с учётом выбранного стиля
  `.h`/`.hpp` и имени класса), а выбранные значения передавать в выбор каталога,
  генерацию `#include` и обновление CMake-файлов.
- [x] Заменить **Tools → Configure shortcut** полноценным редактором сочетаний:
  таблица команд с поиском, колонки Default/Current, захват нажатого сочетания,
  очистка привязки, Reset selected и Reset all. Показывать конфликт прямо в
  диалоге и не разрешать Apply, пока он не устранён; поддержать команды без
  сочетания и явно отмечать зарезервированные клавиши.
- [x] Доработать **Tools → Editor theme**: показывать доступные темы с текущим
  выбором, кратким предпросмотром и кнопками Apply/Reset. Переключать preview
  немедленно, но при Cancel восстанавливать исходную тему; встроенные темы
  оставить неизменяемыми и предусмотреть создание пользовательской копии.
- [x] Заменить последовательные списки **Tools → Editor colors** единым редактором
  всех цветовых ролей. Для каждой роли показывать effective color, override и
  палитру, добавить preview фрагмента C/C++, Reset role/Reset all и учёт числа
  цветов терминала. Изменения применять к открытому редактору для preview, но
  сохранять только после Apply и полностью откатывать при Cancel.
- [x] Хранить shortcuts, theme и colors в пользовательских настройках IDE, чтобы
  эти диалоги работали без открытого проекта; при необходимости разрешить
  отдельные проектные overrides с явным выбором области. Записывать настройки
  атомарно и покрыть редактирование, конфликты, Apply, Cancel и Reset тестами.
- [x] Добавить команды **Previous/Next sidebar tab** и **Previous/Next lower tab**
  в общий реестр настраиваемых shortcuts. В качестве начальных сочетаний
  рассмотреть `Alt+PageUp/PageDown` для боковой и
  `Alt+Shift+PageUp/PageDown` для нижней панели; переключать только видимые
  вкладки по кругу, активировать соответствующую панель и показывать её имя в
  status bar. Отдельно проверить распознавание этих сочетаний терминалами.
- [x] Исправить keyboard mnemonics меню: `Alt+F/E/S/R/P/D/T/W/H` должны открывать
  соответствующее верхнее меню, а подчёркнутая буква внутри открытого меню —
  активировать его пункт без мыши. Зарезервировать эти Meta-клавиши и убрать
  конфликтующие назначения команд (сейчас, например, `Alt+E/R/P/T/W` также
  используются для Problems/Registers/Presets/Target/Watch), предложив им новые
  defaults или переназначение через редактор shortcuts.
- [x] Добавить автоматическую проверку уникальности мнемоник в пределах каждого
  меню и PTY-тесты для открытия всех верхних меню, запуска вложенных пунктов,
  `Esc`/Cancel и терминалов, передающих Alt как ESC-prefix. В справке явно описать
  различие между mnemonic меню и глобальным shortcut команды.
