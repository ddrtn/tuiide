...
Язык меню и панелей переключается через **Tools → Interface language** и сохраняется
в `settings.json` (`language: "en"` или `"ru"`). Диалоги настройки сочетаний,
темы и цветов частично переведены. В остальных диалогах и сообщениях пока может
встречаться английский текст.
